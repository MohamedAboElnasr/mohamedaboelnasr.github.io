...
(function oneko() {
  ...
  // Capture currentScript immediately while it's valid
  ...
  const nekoSpeed = 15; // User requested speed
  const spriteSets = {
    idle: [[-3, -3]],
    alert: [[-7, -3]],
    scratchSelf: [
  ...
  function init() {
    ...
    nekoEl.style.filter = "drop-shadow(0 0 5px #bb86fc)";
  ...
  function idle() {
    ...
    if (
      idleTime > 10 &&
      Math.floor(Math.random() * 50) == 0 && // More active!
      idleAnimation == null
    ) {
